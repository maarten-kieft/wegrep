...
export default function LeagueGamesClient({ leagueId }: { leagueId: string }) {
  ...
  const filteredGames = filterGames(league.games)
  ...
  return (
    <div className="min-h-screen bg-background pb-20 overflow-x-hidden">
      ...
      <div className="px-4 pt-6 max-w-4xl mx-auto">
        <Button variant="ghost" className="text-muted-foreground mb-4" onClick={() => router.push("/leagues")}>
          ← Back to leagues
        </Button>

        <p className="text-muted-foreground text-center mb-6">Select a game from {league.name}</p>
          ...
          {filteredGames.length === 0 ? (
            ...
            filteredGames.map((game) => (
              <Card
                ...
                onClick={() => router.push(`/leagues/${leagueId}/games/${game.id}`)}
              >
